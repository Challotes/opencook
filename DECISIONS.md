...
- **Memory clue is mandatory (settled 2026-04-17).** The passphrase hint field is required in UpgradeModal and ChangePassphraseModal. Submit button disabled until filled. No minimum length — a user who types "x" has made a conscious choice, but skipping entirely is blocked. Rationale: BSVibes has no email/phone recovery channel; the hint is the only lifeline if a user forgets their passphrase.
