...
- `src/lib/in-app-browser.ts` — Pure, SSR-safe UA classifier (`isInAppBrowser` / `classifyInAppBrowser` → `{inApp, app}` / `detectMobileOS`) for in-app social WebViews (Telegram/X/Instagram/Facebook/TikTok/…). The crawler-exempt list is checked FIRST so link-preview bots (Twitterbot/TelegramBot/Discordbot/facebookexternalhit/Googlebot) fall through to `<Feed>` and OG previews still render. **Fail-safe allowlist (2026-06-29 round 2):** beyond the named-app denylist, an iOS UA carrying no real-browser token (`Safari/`/`CriOS`/`FxiOS`/`EdgiOS`/`OPT`/`DuckDuckGo`/`Brave`/…) is treated as a bare WebView → splash, and an empty UA fails safe → splash. This catches **Telegram's iOS WebView, which sends a bare WKWebView UA with NO app token and no `Safari/`** (the denylist alone missed it — found in real-device QA). Installed iOS PWAs share that bare UA, so the splash's `InAppStandaloneGuard` redirects them into the app client-side. Consumed server-side by `page.tsx`. Unit-tested in `in-app-browser.test.ts`.
...
- `src/app/page.tsx` — Main entry (server component; **dynamic** — reads the `user-agent` header to gate in-app social WebViews into `InAppBrowserSplash` BEFORE `<Feed>` mounts, so no key is minted in-app; crawlers + `?continue=1` fall through to `<Feed>`). See DECISIONS "In-app browsers ... splash with a window".
...
- `src/components/InAppBrowserSplash.tsx` — Content-first "splash with a window" shown (via `page.tsx`) when an in-app social WebView is detected. **Server component**, rendered INSTEAD of `<Feed>` so the identity provider never mounts → **no key minted in-app (funds-safe by construction)**. Shows the OpenCook brand + a static read-only preview of the top posts (a window, not a wall) + the CTA + a `?continue=1` misdetect escape. Imports NO identity/boost/deposit surfaces by design. No crypto jargon in the splash copy ("account"/"earnings"). See DECISIONS "In-app browsers ... splash with a window".
- `src/components/InAppBrowserCta.tsx` — The one `'use client'` child of the splash. Android: an "Open in Chrome" `intent://` button (routes out of the WebView → Chrome, which can then open the installed PWA via its WebAPK intent filter). iOS/other: a copy-link button + paste-into-Safari instructions (no programmatic redirect exists on iOS). Touches no identity/spend surfaces.
- `src/components/InAppStandaloneGuard.tsx` — Tiny `'use client'` child of the splash that rescues installed PWAs from the iOS UA collision: an installed iOS home-screen PWA sends a UA IDENTICAL to a bare in-app WebView (both drop `Safari/`), so the server-side iOS fail-safe can't tell them apart. On mount it checks `navigator.standalone === true` (iOS) or `display-mode: standalone` and, if standalone, `window.location.replace("/?continue=1")` → renders `<Feed>` (the guard never re-mounts there, so no loop). Renders null; touches no identity/spend surfaces.
