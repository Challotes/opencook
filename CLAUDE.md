...
- `src/components/GoatModeToast.tsx` — One-time celebratory toast on first auto-flip to Goat Mode after first protect (gated by `opencook_goat_welcome_shown` localStorage flag)
...
- `src/components/InstallBookmark.tsx` — Minimised state of the install pitch. 34×34 chip (30px OpenCook icon inside `border-zinc-800`, geometry matches Ask AI pill exactly) centered in the PostForm footer's `grid-cols-3` row. Tap re-opens the slide-up sheet. Highlight flash fires on sheet→bookmark collapse so the eye tracks the destination.
